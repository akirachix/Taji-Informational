...
const FaqSection = () => {
  ...
  return (
    <div id="faqs">
      <div className="flex flex-col h-auto items-center font-mono w-full px-4 sm:px-[55px]">
        <h2
          className="text-2xl sm:text-3xl md:text-4xl mt-10 mb-5 pb-16 font-extrabold"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-4 sm:gap-y-[68px] sm:gap-x-[177px] w-full">
          {faqData.map((faq, index) => (
            <div key={index} className="flex flex-col w-full">
              <div
                className="bg-gray-100 border border-gray-300 rounded-lg p-4 cursor-pointer hover:shadow-md transition-all"
                onClick={() => toggleFaq(index)}
              >
                <div className="flex justify-between items-center">
                  <span
                    className="text-lg sm:text-xl md:text-[30px] font-bold"
                    style={{ fontFamily: 'Darker Grotesque' }}
                  >
                    {faq.question}
                  </span>
